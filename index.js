...
async function handleExtenderCommand(interaction) {
  console.log(`⏰ Comando /extender ejecutado por ${interaction.user.tag}`);
  ...
  if (now < cooldownEnd) {
    ...
    try {
      return await interaction.reply({ 
        content: `⏰ Espera ${timeLeft} segundos antes de extender otro sorteo.`,
        flags: 64
      });
    } catch (err) {
      console.log("⚠️ Error enviando mensaje de cooldown extender");
    }
    return;
  }
  ...
  try {
    if (interaction.replied || interaction.deferred) {
      console.log("⚠️ Interacción de extender ya procesada anteriormente");
      return;
    }
    ...
    try {
      ...
      console.error("❌ Error obteniendo miembro para extender:", fetchError.message);
      return await interaction.editReply({ 
        content: "❌ No se pudo verificar tus permisos."
      });
    }
    ...
    if (!isOwner && !isAdmin) {
      return await interaction.editReply({ 
        content: "❌ No tienes permisos para extender sorteos."
      });
    }
    ...
    if (giveaway.ended) {
      return await interaction.editReply({ 
        content: "❌ Ese sorteo ya ha terminado."
      });
    
    if (giveaway.guildId !== guild.id) {
      return await interaction.editReply({ 
        content: "❌ Ese sorteo no pertenece a este servidor."
      });
    }
    ...
    try {
      ...
      const tiempoExtraMs = tiempoExtra * 60 * 1000;
      const newEndTime = new Date(giveaway.endTime + tiempoExtraMs);
      
      giveaway.endTime = newEndTime.getTime();
      activeSorteos.set(messageId, giveaway);

      const updatedEmbed = {
        title: `🎉 SORTEO: ${giveaway.objeto}`,
        description: `${giveaway.descripcion}\n\n**¿Cómo participar?**\nReacciona con ${GIVEAWAY_EMOJI} para participar!\n\n**Termina:** <t:${Math.floor(newEndTime.getTime() / 1000)}:R>\n**Organizado por:** <@${giveaway.createdBy}>\n\n⏰ **¡Tiempo extendido por ${tiempoExtra} minutos!**`,
        color: 0xFFA500,
        image: message.embeds[0].image,
        footer: {
          text: `Extendido - Termina el ${newEndTime.toLocaleString('es-ES')}`,
          icon_url: client.user.displayAvatarURL()
        },
      ...
      await interaction.editReply({ 
        content: `✅ ¡Sorteo extendido exitosamente! ⏰\n**Objeto:** ${giveaway.objeto}\n**Tiempo agregado:** ${tiempoExtra} minutos\n**Nueva finalización:** <t:${Math.floor(newEndTime.getTime() / 1000)}:F>`
      });

      await channel.send(`⏰ **¡SORTEO EXTENDIDO!** ⏰\n\n🎉 El sorteo de **${giveaway.objeto}** ha sido extendido por **${tiempoExtra} minutos**\n\n¡Aún puedes participar reaccionando con ${GIVEAWAY_EMOJI}!`);

      console.log(`✅ Sorteo extendido: ${giveaway.objeto} por ${tiempoExtra} minutos`);

    } catch (error) {
      console.error("❌ Error extendiendo sorteo:", error.message);
      await interaction.editReply({ 
        content: "❌ Error extendiendo el sorteo. Verifica que el mensaje existe y es un sorteo válido."
      });
    }

  } catch (err) {
    console.error("❌ Error crítico en comando /extender:", err.message, err.stack);
    
    try {
      if (!interaction.replied && !interaction.deferred) {
        await interaction.reply({ 
          content: "⚠️ Error procesando la extensión del sorteo.",
          flags: 64
        });
      } else if (interaction.deferred) {
        await interaction.editReply({ 
      ...
      console.error("❌ Error crítico manejando error de extender:", criticalError.message);
    }
  }
}
