...
const USER_TRIGGER_COOLDOWN = 10 * 60 * 1000;
...
function setCooldown(alertaId) {
  alertaCooldowns.set(alertaId, Date.now() + ALERT_COOLDOWN_MS);
}

function isOnCooldown(alertaId) {
  const expires = alertaCooldowns.get(alertaId);
  if (!expires) return false;
  if (Date.now() > expires) {
    alertaCooldowns.delete(alertaId);
    return false;
  }
  return true;
}
  ...
  try {
    ...
    if (interaction.deferred || interaction.replied) {
      try {
        ...
        console.error("Error editando reply de alerta:", e);
      }
    } else {
      try {
        ...
        console.error("Error respondiendo interacción de alerta:", e);
      }
    }
  }
}
  ...
  const commands = [
    new SlashCommandBuilder()
      ...
      .addIntegerOption(option =>
        option.setName("id")
          .setDescription("ID de la alerta a enviar (número)")
          .setRequired(true))
      .toJSON(),
